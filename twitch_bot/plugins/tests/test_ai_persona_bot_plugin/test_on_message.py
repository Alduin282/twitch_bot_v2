...
class TestAIPersonaOnMessage(IsolatedAsyncioTestCase):
    ...
    async def test__persona_random_not_mod__do_nothing(self) -> None:
        ...
        message_with_command = self._get_message_mock(command, is_mod=False)

        await plugin._on_message(MagicMock(), message_with_command)

        ai_service.set_random_persona.assert_not_called()
        message_with_command.channel.send.assert_not_awaited()
    ...
    async def test__persona_random_mod__updates_persona(self) -> None:
        ...
        command = plugin.RANDOM_PERSONA_COMMAND
        message_with_command = self._get_message_mock(command, is_mod=True)

        await plugin._on_message(MagicMock(), message_with_command)

        ai_service.set_random_persona.assert_awaited_once()
        message_with_command.channel.send.assert_awaited_once()
    ...
    async def test__unknown_command__do_nothing(self) -> None:
        ...
        message_with_unknown_command = self._get_message_mock(
            f"{plugin.PREFIX} unknown_command"
        )

        await plugin._on_message(MagicMock(), message_with_unknown_command)

        message_with_unknown_command.channel.send.assert_not_awaited()
