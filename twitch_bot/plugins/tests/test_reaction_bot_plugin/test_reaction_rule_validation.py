...
class TestReactionRuleValidation(ReactionRuleTestBase):
    ...
    def test__probability_less_than_zero__value_error(self) -> None:
        with self.assertRaises(ValueError):
            self.create_reaction_rule(reaction_probability=-0.1)

    def test__probability_greater_than_one__value_error(self) -> None:
        with self.assertRaises(ValueError):
            self.create_reaction_rule(reaction_probability=1.1)

    def test__probability_zero__no_errors(self) -> None:
        rule = self.create_reaction_rule(reaction_probability=0.0)
        self.assertEqual(rule.reaction_probability, 0.0)

    def test__probability_one__no_errors(self) -> None:
    ...
    def test__zero_cooldown__no_errors(self) -> None:
        rule = self.create_reaction_rule(cooldown_seconds=0.0)
        self.assertEqual(rule.cooldown_seconds, 0.0)

    def test__empty_triggers__value_error(self) -> None:
        empty_triggers = ()
        with self.assertRaises(ValueError):
            self.create_reaction_rule(triggers=empty_triggers)

    def test__empty_replies__value_error(self) -> None:
        empty_replies = ()
        with self.assertRaises(ValueError):
            self.create_reaction_rule(replies=empty_replies)

    def test__crate_reaction_rule__valid_uuid4(self) -> None:
    ...
    def test__trigger_with_multiple_words__value_error(self) -> None:
